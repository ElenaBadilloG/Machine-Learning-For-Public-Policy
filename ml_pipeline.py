...
def plot_distrib(var):
    ...
    - var (np.array)
    '''
    var.plot.hist(alpha=.3, bins=10)
    sns.kdeplot(var, kernel="gau")
...
def top_corr(df, vars_interest, label, heatmap = True):
    df_subs = df[vars_interest]
    corr = df_subs.corr()
    corr_sort = df_subs.corr()[label].sort_values(ascending = False)
    if heatmap:
        ...
        for i, row in df_subs.iterrows():
            ...
            if dev > thresh:
                if var not in out_lst:
                    out_lst[var] = [("MEAN:" + str(mean), "STD:" + str(std)), ("OUTLIER", \
                                     "INDEX: " ,i, "DEV: ", \dev)]    
                else:
...
def categorize(df, vars_to_cat, bins):
    ...
    for i, var in enumerate(vars_to_cat):
        ...
        elif var == 'num_dep' or  var == 'num_30_59_pastdue' or  var == 'num_90_pastdue' or \
        var == 'num_60_89_pastdue' or var == 'r_estate_loans':
            df[name] = df[var].astype('category')
            df[name_d] = df[name].cat.codes
        else:
            col = pd.cut(df[var], bins[i])
            df[name] = col
...
def plot_cond_mean(df, var, label):
    cols = [var, label]
    df[cols].groupby(cols[0]).mean().plot(kind='bar')
...
def plot_importances(df, features, label):
    ...
    pl.yticks(padding, pot_features[sorted_idx])
...
def split_dataset(df, label, test_size = 0.2):
